...
function acquireNamespaceLock(artifactRoot, namespacePath, options = {}) {
  ...
  ensureExistingPathInside(artifactRoot, namespacePath, "Repository namespace lease");
  return acquireRepositoryLease(namespacePath, options);
}
...
function assertManifestDigest(manifest) {
  if (!manifest || !["0.3", "0.4"].includes(manifest.schema_version) || !manifest.integrity) {
    throw new Error("Repository artifact manifest is not integrity-aware.");
  }
...
function commitManifest(namespacePath, manifest, lease, faultInjectionStage) {
  assertRepositoryLease(lease);
  if (manifest.schema_version === "0.4" && (!manifest.coordination ||
      manifest.coordination.lease_id !== lease.lease_id || manifest.coordination.fencing_token !== lease.fencing_token)) {
    throw new Error("Manifest fencing token does not match the active repository lease.");
  }
  const paths = manifestPaths(namespacePath);
  const digest = assertManifestDigest(manifest);
  const current = fs.existsSync(paths.manifest) ? readJson(paths.manifest) : null;
  const currentDigest = current ? assertManifestDigest(current) : "none";
  if (currentDigest !== digest && (manifest.previous_manifest_sha256 !== currentDigest ||
      manifest.manifest_revision !== (current ? current.manifest_revision + 1 : 1))) {
    throw new Error(`Manifest compare-and-swap conflict at revision ${manifest.manifest_revision}.`);
  }
  ...
  if (fs.existsSync(historyPath)) {
    ...
    try {
      atomicCreateJson(historyPath, manifest);
    } catch (error) {
      if (error.code !== "EEXIST") throw error;
      const existing = readJson(historyPath);
      if (assertManifestDigest(existing) !== digest) throw new Error(`Manifest history conflict at revision ${manifest.manifest_revision}.`);
    }
  }
  if (faultInjectionStage === "history_created") {
    throw new Error("Injected artifact transaction failure after manifest history creation.");
  }
  assertRepositoryLease(lease);
  const latest = fs.existsSync(paths.manifest) ? readJson(paths.manifest) : null;
  const latestDigest = latest ? assertManifestDigest(latest) : "none";
  if (latestDigest !== currentDigest && latestDigest !== digest) {
    throw new Error(`Manifest head changed before revision ${manifest.manifest_revision} commit.`);
  }
...
function finalizeReservedManifest(namespacePath, manifest, lease) {
  assertRepositoryLease(lease);
  const paths = manifestPaths(namespacePath);
  const digest = assertManifestDigest(manifest);
  const current = fs.existsSync(paths.manifest) ? readJson(paths.manifest) : null;
  const currentDigest = current ? assertManifestDigest(current) : "none";
  if (manifest.previous_manifest_sha256 !== currentDigest ||
      manifest.manifest_revision !== (current ? current.manifest_revision + 1 : 1)) {
    throw new Error(`Reserved manifest compare-and-swap conflict at revision ${manifest.manifest_revision}.`);
  }
  const historyPath = manifestHistoryFile(namespacePath, manifest.manifest_revision);
  if (!fs.existsSync(historyPath) || assertManifestDigest(readJson(historyPath)) !== digest) {
    throw new Error(`Reserved manifest history mismatch at revision ${manifest.manifest_revision}.`);
  }
  assertRepositoryLease(lease);
  const latest = fs.existsSync(paths.manifest) ? readJson(paths.manifest) : null;
  const latestDigest = latest ? assertManifestDigest(latest) : "none";
  if (latestDigest !== currentDigest && latestDigest !== digest) {
    throw new Error(`Manifest head changed before reserved revision ${manifest.manifest_revision} finalization.`);
  }
  atomicWriteJson(paths.manifest, manifest);
  atomicWrite(paths.sidecar, `${digest}\n`);
  return digest;
...
function fencedManifest(manifest, lease) {
  return finalizeManifest({
    ...manifest,
    schema_version: "0.4",
    isolation: {
      ...manifest.isolation,
      cross_process_manifest_lease: true,
      fencing_tokens: true,
      write_ahead_journal: true
    },
    coordination: {
      backend: "shared_filesystem_lease",
...
function migrateLegacyManifest(repository, namespaceRoot, namespacePath, manifest, now, lease) {
  if (!manifest || manifest.schema_version === "0.4") return manifest;
  if (!["0.2", "0.3"].includes(manifest.schema_version)) throw new Error(`Unsupported repository artifact manifest version: ${manifest.schema_version}`);
  ...
  if (manifest.schema_version === "0.2") {
    ...
    manifest = finalizeManifest({
      ...
      isolation: { ...manifest.isolation, write_ahead_journal: true },
      integrity: {
        algorithm: "sha256",
        canonical_manifest_sha256: "",
    ...
    if (!fs.existsSync(historyPath)) atomicCreateJson(historyPath, manifest);
    atomicWriteJson(manifestPaths(namespacePath).manifest, manifest);
    atomicWrite(manifestPaths(namespacePath).sidecar, `${manifest.integrity.canonical_manifest_sha256}\n`);
  }
  const previousDigest = assertManifestDigest(manifest);
  const migrated = finalizeManifest({
    ...
    integrity: {
      algorithm: "sha256",
      canonical_manifest_sha256: "",
      history_start_revision: manifest.integrity.history_start_revision,
      history_length: manifest.integrity.history_length + 1,
      pending_transaction_count: 0,
      sidecar_required: true
    },
    updated_at: now
  });
  commitManifest(namespacePath, migrated, lease);
  return migrated;
}

function buildManifest(repository, namespaceRoot, existingManifest, artifactEntry, now, lease) {
...
function recoverPendingTransactionsLocked(artifactRoot, namespacePath, lease) {
  const recovered = [];
  for (const journalPath of listPendingJournals(namespacePath)) {
    ...
    if (artifactHash === journal.artifact_sha256) {
      ...
      if (current.digest !== candidateDigest) {
        const historyPath = manifestHistoryFile(namespacePath, journal.candidate_manifest.manifest_revision);
        if (fs.existsSync(historyPath) && assertManifestDigest(readJson(historyPath)) === candidateDigest) {
          finalizeReservedManifest(namespacePath, journal.candidate_manifest, lease);
        } else {
          const candidate = fencedManifest(journal.candidate_manifest, lease);
          commitManifest(namespacePath, candidate, lease);
        }
      }
...
function collectIntegrityIssues(repository, artifactRoot, namespacePath) {
  ...
  if (manifest.schema_version === "0.4" && (!manifest.coordination || !Number.isInteger(manifest.coordination.fencing_token))) {
    issues.push({ code: "MANIFEST_FENCING_TOKEN_MISSING", message: "v0.4 manifest does not identify its committing lease and fencing token." });
  }

  const start = manifest.integrity.history_start_revision;
  let previous = "none";
  let historyCount = 0;
  let lastFencingToken = 0;
  ...
  for (let revision = start; revision <= manifest.manifest_revision; revision += 1) {
    ...
    try {
      ...
      if (historic.schema_version === "0.4") {
        const fencingToken = historic.coordination && historic.coordination.fencing_token;
        const leaseId = historic.coordination && historic.coordination.lease_id;
        if (!Number.isInteger(fencingToken) || fencingToken < lastFencingToken) {
          issues.push({ code: "MANIFEST_FENCING_TOKEN_NOT_MONOTONIC", message: `Manifest fencing token regressed at revision ${revision}.` });
        } else if (fencingToken === lastFencingToken && lastLeaseId !== null && leaseId !== lastLeaseId) {
          issues.push({ code: "MANIFEST_FENCING_TOKEN_REUSED", message: `Manifest fencing token was reused by a different lease at revision ${revision}.` });
        }
  ...
  if (manifest.schema_version === "0.4" && manifest.coordination.fencing_token !== lastFencingToken) {
    issues.push({ code: "MANIFEST_FENCING_HEAD_MISMATCH", message: "Current manifest fencing token does not match the history head." });
  }
  const fencingCounterPath = path.join(namespacePath, ".fencing-token");
  const fencingCounter = fs.existsSync(fencingCounterPath) ? Number(fs.readFileSync(fencingCounterPath, "utf8").trim()) : 0;
  if (manifest.schema_version === "0.4" && (!Number.isSafeInteger(fencingCounter) || fencingCounter < manifest.coordination.fencing_token)) {
    issues.push({ code: "FENCING_COUNTER_ROLLBACK", message: "Fencing-token counter is lower than the committed manifest token." });
  }
  ...
  if (options.recover === true) {
    ...
    try {
      ...
      if (fs.existsSync(paths.manifest)) {
        ...
        if (existing.schema_version === "0.2") {
          migrateLegacyManifest(repository, path.join("repositories", repository.key), namespacePath, existing, new Date().toISOString(), lock);
        }
      }
      recovered = recoverPendingTransactionsLocked(artifactRoot, namespacePath, lock);
      if (fs.existsSync(paths.manifest)) {
        migrateLegacyManifest(repository, path.join("repositories", repository.key), namespacePath, readJson(paths.manifest), new Date().toISOString(), lock);
      }
    } finally {
...
function persistRepositoryArtifact(options, content, fileName, contentType) {
  ...
  try {
    ...
    if (existingManifest && existingManifest.schema_version === "0.2") {
      existingManifest = migrateLegacyManifest(repository, namespaceRoot, namespacePath, existingManifest, new Date().toISOString(), lock);
    }
    recoverPendingTransactionsLocked(artifactRoot, namespacePath, lock);
    ...
    const manifest = buildManifest(repository, namespaceRoot, existingManifest, artifactEntry, now, lock);
    const paths = manifestPaths(namespacePath);
    ...
    commitManifest(namespacePath, manifest, lock, options.faultInjectionStage);
...
function parseArgs(argv) {
  ...
  const valueOptions = new Set(["repository", "artifact-root", "mission", "wave", "kind", "artifact-id", "input", "source", "content-type", "lease-timeout-ms", "lease-ttl-ms", "lock-timeout-ms", "lock-stale-ms"]);
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg.startsWith("--") && valueOptions.has(arg.slice(2))) {
      ...
      if (["--lease-timeout-ms", "--lease-ttl-ms", "--lock-timeout-ms", "--lock-stale-ms"].includes(arg)) {
...
function main() {
  try {
    ...
    if (!options.repository || !options.mission || !options.wave || !options.kind || !options.artifactId) {
      throw new Error("Usage: node repository-artifact-store.js --repository <repo> --mission <id> --wave <id> --kind <kind> --artifact-id <id> [--input <json> | --source <file>] [--content-type <type>] [--artifact-root <dir>] [--overwrite] [--lease-timeout-ms <ms>] [--lease-ttl-ms <ms>]");
    }
