...
| SLSA artifact verification: https://slsa.dev/spec/v1.2/verifying-artifacts | Root-of-trust and expectation checks | Verify trusted identity and signature, then compare provenance fields with preconfigured expected source, type, and parameters | `VerifierTrustPolicy` supplies roots while the checkpoint, plan, and report supply exact expected campaign, repository, candidate, and evaluator values | `schema-files/verifier-trust-policy.schema.json`, `comparative-evaluation-attestation-runner.js` |
| Sigstore keyless signing overview: https://docs.sigstore.dev/cosign/signing/overview/ | Workload identity and transparency | Short-lived certificates can bind ephemeral signing keys to OIDC identity and record signing events in a transparency log | Defines the next external-authentication layer; local `remote` and independence claims are not promoted to trusted-execution facts | `runtime-automation-roadmap.md`, `bounded-self-improvement-operations.md` |
