...
| IETF RFC 8705 OAuth 2.0 mutual-TLS client authentication and certificate-bound access tokens: https://www.rfc-editor.org/rfc/rfc8705.html | Sender-constrained credentials | A client proves possession of the certificate private key and the resource server checks that the access token is bound to that certificate | A managed identity adapter must bind credential and proof evidence to the exact principal and reject bearer-only substitution before gateway admission | `protected-tool-gateway-contract.md`, `schema-files/tool-gateway-request.schema.json` |
| OAuth 2.0 Security Best Current Practice, RFC 9700: https://www.rfc-editor.org/rfc/rfc9700.html | Audience restriction, sender constraint, and least privilege | Access credentials should be audience-restricted, sender-constrained where feasible, and limited to the minimum resources and operations | Bind one principal and gateway audience to one exact tool, input digest, operation class, validity window, and finite dispatch authority | `protected-tool-gateway-contract.md`, `schema-files/tool-gateway-request.schema.json`, `schema-files/tool-gateway-decision.schema.json` |
