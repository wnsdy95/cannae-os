...
| in-toto Statement v1: https://github.com/in-toto/attestation/blob/main/spec/v1/statement.md | Subject and predicate binding | Match immutable subjects by digest and identify the assertion semantics with an unambiguous predicate type | The report artifact hash is the subject; report self-digest, plan, set, lineage, evaluator, repository, and validity claims are the comparative predicate | `bounded-self-improvement-operations.md`, `schema-files/comparative-evaluation-attestation.schema.json` |
...
| Sigstore bundle format: https://docs.sigstore.dev/about/bundle/ and verification guide: https://docs.sigstore.dev/cosign/verifying/verify/ | Portable verification material | A verification bundle carries signature and verification material, while policy still checks expected identity and issuer; transparency material extends verification beyond short certificate lifetime | Phase 11A uses a provider-neutral local evidence schema; Phase 11B will ingest native bundles through official Sigstore libraries instead of claiming wire compatibility | `runtime-automation-roadmap.md`, `schema-files/verifier-identity-evidence.schema.json` |
