...
| GitHub Actions OIDC reference: https://docs.github.com/en/actions/reference/security/oidc and artifact attestations: https://docs.github.com/en/actions/concepts/security/artifact-attestations | Workflow identity and signed build claims | OIDC subjects and claims bind repository, ref, workflow and environment context; artifact attestations bind signed provenance to an artifact but still require policy verification | Define exact GitHub issuer, subject, audience, repository ID, workflow ref, commit, ref and runner-environment claims behind a provider adapter | `docs/verifier-execution-integrity.md`, `schema-files/verifier-runtime-policy.schema.json` |
