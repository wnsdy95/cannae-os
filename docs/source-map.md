...
| OpenAI Learn, learn.chatgpt.com | Codex hook lifecycle and enforcement boundary | `PreToolUse` can deny or rewrite covered local tool calls, match all supported tools, and expose stable event/tool/input fields, while project hooks require trust and do not cover every hosted or specialized execution path | Use Codex hooks as a local adapter for a plan-digest-authorized policy and result-bound lease state machine; require explicit lineage continuation after resume and retain an external or managed gateway as the stronger deployment boundary | `docs/enforced-dispatch-and-resume.md`, `dispatch-runtime-controller.js`, `dispatch-hook-adapter.js`, `install-dispatch-hooks.js`, `skill-mission-controller.js` |
| Anthropic Claude Code docs, code.claude.com | Claude Code hooks, settings precedence, and subagent identity | `PreToolUse` can deny covered calls and exposes an `agent_id` for subagents, but client-side project settings and non-hooked paths are not a complete authority boundary | Bind provider agent identity when available, require exact pre/post tool correlation, interrupt authority on session start/resume, and use managed settings or an external gateway for stronger enforcement | `docs/enforced-dispatch-and-resume.md`, `dispatch-runtime-controller.js`, `dispatch-hook-adapter.js`, `install-dispatch-hooks.js`, `skill-mission-controller.js` |
...
| IETF JSON Web Token RFC 7519: https://www.rfc-editor.org/rfc/rfc7519.html and JSON Web Key RFC 7517: https://www.rfc-editor.org/rfc/rfc7517.html | Offline JWT/JWK verification and claim validation | A relying party must verify the signature under an accepted algorithm/key and independently validate issuer, audience and temporal claims; a JWK `kid` selects key material but is not authority by itself | Accept only exact `RS256`, `kid`, issuer, audience, subject and bounded time claims under the exact manifest-approved provider JWKS artifact | `docs/github-actions-native-verifier-adapter.md`, `docs/gitlab-ci-native-verifier-adapter.md`, `github-actions-oidc.js`, `gitlab-ci-oidc.js` |
| GitLab ID token claims: https://docs.gitlab.com/ci/secrets/id_token_authentication/, `id_tokens`: https://docs.gitlab.com/ci/yaml/#id_tokens, cloud trust: https://docs.gitlab.com/ci/cloud_services/, runner model: https://docs.gitlab.com/runner/, hosted runners: https://docs.gitlab.com/ci/runners/hosted_runners/, live discovery: https://gitlab.com/.well-known/openid-configuration, and live JWKS: https://gitlab.com/oauth/discovery/keys | Pipeline, config, and runner identity | GitLab.com signs `RS256` ID tokens with stable source/job project and namespace IDs, protected-ref state, config ref/SHA, commit, runner ID and hosted/self-hosted class; stable IDs should accompany paths, while a runner ID does not prove physical topology | Verify the compact JWT against a manifest-pinned GitLab.com JWKS, require equal stable source/job identity, protected same-project config and clean commit, and keep runner/infrastructure topology conservative | `docs/verifier-execution-integrity.md`, `docs/gitlab-ci-native-verifier-adapter.md`, `schema-files/gitlab-ci-oidc-trust-bundle.schema.json`, `schema-files/gitlab-ci-oidc-evidence.schema.json`, `gitlab-ci-oidc.js` |
...
| IETF RFC 9449 DPoP nonce and replay controls, sections 8 and 11.1: https://www.rfc-editor.org/rfc/rfc9449.html#section-11.1 | Proof of possession, server challenge, bounded proof lifetime and replay state | Bind a proof to the request target, reject a response whose nonce does not match a recently supplied server nonce, limit proof lifetime, and track proof identifiers or equivalent state to prevent reuse | Bind the authenticated principal to one gateway audience and tool transaction; exact nonce equality, manifest state, idempotency, and single-use dispatch binding reject stale and replayed responses | `docs/verifier-pre-dispatch-challenge.md`, `docs/protected-tool-gateway-contract.md`, `schema-files/verifier-challenge-set.schema.json`, `schema-files/tool-gateway-request.schema.json` |
...
| SPIFFE X.509-SVID specification: https://spiffe.io/docs/latest/spiffe-specs/x509-svid/ | Workload identity certificate profile | An X.509-SVID leaf carries exactly one SPIFFE ID URI SAN, follows a non-root trust-domain path, and is validated to a configured bundle | Trust policies pin the root and exact SPIFFE ID; verifier and gateway admission reject missing, extra, malformed, wrong-domain, wrong-path, inactive, revoked, or untrusted leaf identities | `bounded-self-improvement-operations.md`, `gateway-identity-admission.md`, `schema-files/verifier-trust-policy.schema.json`, `schema-files/gateway-identity-policy.schema.json`, `verifier-identity-evidence.js`, `gateway-identity-evidence.js` |
| SPIFFE Workload API specification: https://github.com/spiffe/spiffe/blob/main/standards/SPIFFE_Workload_API.md | Short-lived workload credentials and trust bundles | Workloads obtain rotating X.509-SVIDs and trust bundles from a local workload endpoint instead of storing long-lived identity credentials | Defines the production credential-delivery boundary; the current provider-neutral evidence verifiers consume supplied TLS/PEM state and do not implement the Workload API client | `runtime-automation-roadmap.md`, `bounded-self-improvement-operations.md`, `gateway-identity-admission.md` |
...
| NIST SP 800-207, Zero Trust Architecture: https://csrc.nist.gov/pubs/sp/800/207/final | Per-session resource authorization | Network location or asset ownership creates no implicit trust; subject and device authentication and authorization precede a resource session | Require exact authenticated principal, current authority, repository state, and tool binding before each gateway transaction instead of trusting the agent process or prior session | `protected-tool-gateway-contract.md`, `enforced-dispatch-and-resume.md`, `protected-tool-gateway.js` |
| NIST SP 800-207A, cloud-native application identity: https://csrc.nist.gov/pubs/sp/800/207/a/final | Application and service identity enforcement | API gateways, sidecars, and service identities can enforce granular policy between application services independently of network location | Put provider-specific tool adapters behind one identity-aware execution boundary and keep the common request/decision/receipt contract provider-neutral | `protected-tool-gateway-contract.md`, `reference-architecture.md` |
| NIST SP 800-53 Rev. 5.1, AU-3 and AU-9: https://csrc.nist.gov/pubs/sp/800/53/r5/upd1/final | Audit record content and protection | Audit records identify event type, time, source, outcome, and actor, and audit information is protected from unauthorized access, modification, and deletion | Persist identity-bound request, decision, result, reason, timestamp, and append-only predecessor references in the verified repository manifest | `protected-tool-gateway-contract.md`, `schema-files/tool-gateway-transaction-event.schema.json`, `repository-artifact-store.js` |
| IETF TLS 1.3 RFC 8446: https://www.rfc-editor.org/rfc/rfc8446.html | Authenticated transport and exporter derivation | Certificate, CertificateVerify, and Finished messages authenticate possession within the negotiated handshake, and exporters derive channel-specific keying material | Require an authorized TLS 1.3 client certificate at the gateway-side socket and bind the resulting observation to one signed transaction artifact | `gateway-identity-admission.md`, `gateway-identity-adapter.js`, `run-gateway-identity-adapter-fixtures.js` |
| IETF TLS 1.3 channel bindings RFC 9266: https://www.rfc-editor.org/rfc/rfc9266.html | Higher-layer channel binding | `EXPORTER-Channel-Binding` with empty context and 32 bytes yields a channel binding that is unique to the negotiated TLS connection | Hash the exporter into the authenticated-principal proof and require exact equality with the gateway-signed TLS observation | `gateway-identity-admission.md`, `gateway-identity-adapter.js`, `schema-files/gateway-principal-evidence.schema.json` |
| Node.js TLS `exportKeyingMaterial`: https://nodejs.org/api/tls.html#tlssocketexportkeyingmateriallength-label-context | Runtime TLS observation | The TLS socket API exposes protocol, authorization, peer/local certificates, cipher, and RFC 5705-compatible exporter material after handshake | Read proof from the server-side `TLSSocket`; do not accept agent-declared certificate or exporter values as identity evidence | `gateway-identity-admission.md`, `gateway-identity-adapter.js`, `run-gateway-identity-adapter-fixtures.js` |
| IETF RFC 8705 OAuth 2.0 mutual-TLS client authentication and certificate-bound access tokens: https://www.rfc-editor.org/rfc/rfc8705.html | Sender-constrained credentials | A client proves possession of the certificate private key and the resource server obtains the client certificate from the TLS stack before checking certificate-bound use | Bind the gateway-side TLS certificate and exporter observation to the exact principal projection and reject bearer-only or caller-declared substitution before admission | `protected-tool-gateway-contract.md`, `gateway-identity-admission.md`, `schema-files/tool-gateway-request.schema.json`, `schema-files/gateway-principal-evidence.schema.json` |
| OAuth 2.0 Security Best Current Practice, RFC 9700: https://www.rfc-editor.org/rfc/rfc9700.html | Audience restriction, sender constraint, and least privilege | Access credentials should be audience-restricted, sender-constrained where feasible, and limited to the minimum resources and operations; resource servers enforce proof and replay restrictions | Bind one principal, gateway audience, one-use challenge and TLS proof to one exact tool, input digest, operation class, validity window, and finite dispatch authority | `protected-tool-gateway-contract.md`, `gateway-identity-admission.md`, `schema-files/tool-gateway-request.schema.json`, `schema-files/tool-gateway-decision.schema.json` |
| etcd v3.6 API: https://etcd.io/docs/v3.6/learning/api/ | Lease, keepalive, revision, transaction | Expiring ownership requires renewal; protected updates need atomic comparison and a monotonic ordering token | The shared-filesystem backend uses expiring leases, monotonic fencing tokens, and immutable revision compare-and-swap; stronger deployments must use a linearizable coordinator | `repository-artifact-isolation-policy.md`, `protected-tool-gateway-contract.md`, `repository-lease.js`, `repository-artifact-store.js` |
| Node.js `child_process.spawn`: https://nodejs.org/api/child_process.html | Exact process invocation primitive | `spawn` separates the command from its argument array and exposes exact cwd, environment, stdio, shell, detached and timeout options | Execute only a policy-fixed canonical ELF or Mach-O executable and argv with `shell: false`, empty environment, ignored stdin, bounded output and timeout | `protected-process-execution.md`, `protected-process-executor.js`, `schema-files/protected-executor-policy.schema.json` |
| MITRE CWE-78: https://cwe.mitre.org/data/definitions/78.html | OS command injection | Externally influenced data must not alter the intended command or its arguments | Accept only a manifest policy reference and rule ID at runtime; never accept an agent-built command string, option, path or environment fragment | `protected-process-execution.md`, `schema-files/protected-process-tool-input.schema.json`, `protected-process-executor.js` |
| Linux `no_new_privs`: https://www.kernel.org/doc/html/latest/userspace-api/no_new_privs.html | Privilege-gain prevention | `no_new_privs` prevents `execve` from granting privilege but is not a complete confinement mechanism | Record privilege control as absent in the reference adapter and require direct provider evidence before making a stronger claim | `protected-process-execution.md`, `schema-files/protected-executor-policy.schema.json` |
| Linux seccomp filter: https://www.kernel.org/doc/html/v5.9/userspace-api/seccomp_filter.html | Syscall filtering | Seccomp filters restrict syscall behavior and have explicit privilege and synchronization requirements | Keep `syscall_filter: false` in the POSIX reference profile; a future Linux provider must observe and bind the actual filter | `protected-process-execution.md`, `schema-files/protected-executor-policy.schema.json` |
| OCI Runtime Specification, Linux: https://github.com/opencontainers/runtime-spec/blob/main/config-linux.md | Linux container isolation configuration | Namespaces, devices, resources, seccomp, masked paths and related controls are distinct runtime configuration | Treat sandbox assurance as a measured provider profile rather than infer it from executable, runtime or image digests | `protected-process-execution.md`, `runtime-automation-roadmap.md` |
| NIST SP 800-190: https://csrc.nist.gov/pubs/sp/800/190/final | Application container security | Image, registry, orchestrator, runtime and host risks require distinct controls across the container lifecycle | Phase 17B2B must retain and appraise each applicable container layer and cannot upgrade assurance from one image digest | `protected-process-execution.md`, `research-compendium.md`, `runtime-automation-roadmap.md` |
...
| Operational skill mission lifecycle | mission command, confirmation, execution reporting, and AAR feedback | Intent and authority boundaries must remain connected to subordinate understanding, execution evidence, assessment, and follow-on learning | `skill-mission-controller.js` automatically generates every current-wave routing receipt, blocks incomplete preflight, issues digest-bound role context, verifies repository evidence, records SITREP/AAR, and queues bounded improvement without granting AI approval or release authority | `skill-operational-mission-lifecycle.md`, `schema-files/mission-wave-plan.schema.json`, `schema-files/agent-context-pack.schema.json`, `schema-files/mission-wave-report.schema.json`, `schema-files/mission-wave-closeout.schema.json`, `run-skill-mission-controller-fixtures.js` |
| Enforced dispatch and resumable execution artifact | precombat checks, dispatch control, interruption drills, and relief-in-place accountability | A routing receipt or context pack proves preparation, not current authority to invoke a tool; dispatch authority must be preauthorized by exact policy-draft digest, finite, repository-bound, replay-resistant, interruptible, and explicitly continued after resume | Compiles only a USER-plan-authorized draft, issues one manifest-backed lease lineage per mission agent, serializes nonterminal lease handoff across one repository namespace, reloads exact policy and mission evidence before each covered call, records exact pre/post correlation and result-bound checkpoints, gates wave reporting on settled lineages, denies drift/replay/default-unknown tools, and never renews authority from session restoration alone | `docs/enforced-dispatch-and-resume.md`, `dispatch-runtime-controller.js`, `dispatch-hook-adapter.js`, `install-dispatch-hooks.js`, `skill-mission-controller.js`, `schema-files/dispatch-tool-policy.schema.json`, `schema-files/agent-dispatch-lease.schema.json`, `schema-files/tool-admission-event.schema.json`, `schema-files/agent-execution-checkpoint.schema.json`, `run-dispatch-runtime-fixtures.js`, `run-skill-mission-controller-fixtures.js` |
...
- `gateway-identity-evidence.js`
- `gateway-identity-adapter.js`
- `schema-files/gateway-identity-policy.schema.json`
- `schema-files/gateway-identity-challenge.schema.json`
- `schema-files/gateway-principal-evidence.schema.json`
