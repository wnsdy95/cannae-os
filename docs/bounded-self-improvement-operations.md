...
The common adapter verifies a policy-pinned builder assertion. Phase 14 additionally verifies native GitHub Actions and GitLab.com OIDC tokens. It does not yet support enterprise/self-managed issuers, independently attest a self-hosted runner or local host, or appraise TEE vendor evidence. See `docs/verifier-execution-integrity.md` for provider boundaries.
...
### 2.4.4 Failure-domain independence

`VerifierTrustPolicy` v0.6 retains identity, execution, and challenge assurance and replaces declared independence labels with computed correlation domains. `VerifierRuntimePolicy` v0.2 records provider, operator, control-plane, account, project, runner-pool, infrastructure, region, and zone IDs for every profile. Any shared required component creates a correlation edge, and correlation is transitive.

Pre-dispatch readiness computes deterministic `VID-*` domains from the complete runtime policy. `VerifierExecutionEvidence` v0.2 carries the adapter-observed identities under the same builder-and-verifier DSSE signatures as the execution environment. Receipt and comparative quorum checks use the resulting execution domain instead of `verifier.independence_group`. Unknown or incomplete identity, a profile/evidence mismatch, graph substitution, or too few domains fails closed. See `docs/verifier-independence-assurance.md`.

### 2.4.5 Continuous transparency state

`VerifierTrustPolicy` v0.7 retains every v0.6 assurance and adds one exact manifest-backed `TransparencyPolicy`, state stream, and freshness limit. Each `TransparencyState` advances from the immediately previous state using signed log checkpoints, RFC 6962 consistency proofs, distinct witness/monitor operator quorums, sequential dual-threshold TUF root changes, and immutable incident/revocation history.

The supervisor reloads the complete state sequence and every embedded root, observation, rotation, and incident from the verified repository manifest. Only the newest state may authorize dispatch; it must be `ready`, unexpired, within both configured age limits, rooted in the verifier policy's admitted trust set, and free of active incident or revocation blocks. A blocked historical state stays in the chain so recovery cannot erase the failure. Cycle-order v0.7 records the exact policy/state references, sequence, generation time, age bound, observer and incident counts, and transparency validity boundary. See `docs/transparency-operations.md`.
...
7. For trust-policy v0.2+, persist fresh adapter-specific workload identity evidence from every verifier needed for quorum; Sigstore verifiers also require the exact selected TrustedRoot artifact. For v0.4+, persist and bind the human-approved runtime policy with a complete profile assignment for every verifier. For v0.5+, first run the supervisor to issue the challenge, then produce fresh identity evidence with each assigned nonce and rerun supervision before work. For v0.6+, use runtime-policy v0.2+ and require enough computed failure domains. Runtime-policy v0.3 GitHub Actions and GitLab CI profiles also require manifest-backed provider-specific OIDC evidence and JWKS trust material after execution. For v0.7, persist a complete current transparency state before supervision.
...
1. For trust-policy v0.2 through v0.4, refresh and persist verifier workload identity evidence before it expires; refresh a Sigstore TrustedRoot or change a v0.4+ runtime policy only through a separately authorized trust change. Under v0.5+, do not preselect a nonce: let the supervisor issue the current challenge, then persist evidence containing each exact assigned nonce. Under v0.6+, treat every failure-domain identity change as a runtime-policy trust change. Under v0.7, advance the transparency state with fresh checkpoint, consistency, witness, monitor, root and incident evidence before its age or validity boundary.
2. Run the campaign supervisor against the verified artifact store. Under v0.5+, expect the challenge-issuing call to hold and rerun after responses arrive. Stop unless the final order is current and `ready`, every required identity, challenge, independence, and transparency admission is satisfied, and `valid_until` has not been reached.
...
- `execution_origin: remote` is a signed verifier claim, not proof of a trusted execution environment. Under v0.6, independence instead comes from the complete adapter-verified component identity and computed `VID-*` graph; even that remains dependent on the trusted builder/provider adapter.
...
- Trust-policy v0.2 binds a current SPIFFE workload to a static verifier key and transparency checkpoint. Trust-policy v0.6 additionally blocks label-only independence and shared known failure domains, but still does not prove honest verifier logic, protected host execution, truthful claims from a compromised builder, or absence of every upstream common-mode dependency.
...
- Trust-policy v0.7 adds persistent local consistency state, witness/monitor signatures, TUF root rotation and incident handling. It still does not operate a Rekor/TUF service, collect checkpoints, discover shards, run independent observers, or provide gossip; withheld split views remain an external detection problem.
