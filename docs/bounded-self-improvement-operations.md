...
The common adapter verifies a policy-pinned builder assertion. It does not yet verify native GitHub or GitLab OIDC tokens, independently attest a local host, or appraise TEE vendor evidence. See `docs/verifier-execution-integrity.md` for provider boundaries and Phase 12B/12C requirements.
...
7. For trust-policy v0.2+, persist fresh adapter-specific workload identity evidence from every verifier needed for quorum; Sigstore verifiers also require the exact selected TrustedRoot artifact. For v0.4, persist and bind the human-approved runtime policy with a complete profile assignment for every verifier.
...
1. For trust-policy v0.2+, refresh and persist verifier workload identity evidence before it expires; refresh a Sigstore TrustedRoot or change a v0.4 runtime policy only through a separately authorized trust change.
