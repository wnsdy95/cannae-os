...
1. For trust-policy v0.2 through v0.4, refresh and persist verifier workload identity evidence before it expires; refresh a Sigstore TrustedRoot or change a v0.4+ runtime policy only through a separately authorized trust change. Under v0.5+, do not preselect a nonce: let the supervisor issue the current challenge, then persist evidence containing each exact assigned nonce. Under v0.6, treat every failure-domain identity change as a runtime-policy trust change.
2. Run the campaign supervisor against the verified artifact store. Under v0.5+, expect the challenge-issuing call to hold and rerun after responses arrive. Stop unless the final order is current and `ready`, trust, identity, required challenge, and required independence admission are satisfied, and `valid_until` has not been reached.
