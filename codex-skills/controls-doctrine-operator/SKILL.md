...
5. Select the execution path from the exact raw input. For ordinary integration inputs, run `begin` immediately before the external adapter acts and `commit` with exact none sentinels for bounded-execution references. For `ProtectedProcessToolInput`, never run manual `begin` or accept a caller-declared result: run `scripts/operate_protected_executor.js execute`, which reloads the policy, owns begin, persists a signed envelope before spawn, executes only the policy-fixed ELF or Mach-O executable and argv, reappraises that executable after close, persists a signed observation, and submits all three exact evidence references.
...
Phase 17B1 proves authenticated-reference identity. Phase 17B2A executes one policy-pinned local process but does not provide filesystem, syscall, privilege, process-tree, or network isolation and does not prove an exclusive deployment. `managed_exclusive`, production execution, and release remain denied. Read `docs/gateway-identity-admission.md`, `docs/protected-tool-gateway-contract.md`, and `docs/protected-process-execution.md` before integrating an adapter.
...
- The protected process reference adapter fixes one executable and argv, persists signed before/after evidence, and prevents automatic rerun after a claim. It does not isolate filesystems, syscalls, privileges, descendant processes, or network access; never describe it as a sandbox, production executor, or exclusive tool path.
