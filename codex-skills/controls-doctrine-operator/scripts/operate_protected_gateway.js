...
function findRuntimeRoot() {
  ...
  for (const candidate of candidates.filter(Boolean)) {
    ...
    if (fs.existsSync(path.join(root, "protected-tool-gateway.js")) &&
        fs.existsSync(path.join(root, "docs", "source-map.md"))) return root;
  }
  throw new Error("Cannae OS protected gateway runtime not found. Reinstall the skill or set CANNAE_OS_HOME.");
}
