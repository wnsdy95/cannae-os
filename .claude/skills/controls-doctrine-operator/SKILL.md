...
5. For `runtime_control` or `skill`, pre-persist one sealed comparative set and plan, then run the identical harness against the accepted baseline and candidate worktrees. Require a `promotable` manifest-backed report; rollback on measured regression and escalate on an inconclusive comparison.
