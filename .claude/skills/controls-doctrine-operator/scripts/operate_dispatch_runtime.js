...
function findRuntimeRoot() {
  ...
  for (const candidate of candidates.filter(Boolean)) {
    ...
    if (fs.existsSync(path.join(root, "dispatch-runtime-controller.js")) &&
        fs.existsSync(path.join(root, "docs", "source-map.md"))) return root;
  }
  throw new Error("Cannae OS dispatch runtime not found. Reinstall the skill or set CANNAE_OS_HOME.");
}
