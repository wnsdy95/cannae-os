...
function findRuntimeRoot() {
  ...
  for (const candidate of candidates.filter(Boolean)) {
    ...
    if (fs.existsSync(path.join(root, "skill-mission-controller.js")) &&
        fs.existsSync(path.join(root, "docs", "source-map.md"))) {
      return root;
    }
  }
  throw new Error("Cannae OS runtime not found. Reinstall the skill or set CANNAE_OS_HOME.");
}

function main() {
  try {
    const root = findRuntimeRoot();
    const result = spawnSync(process.execPath, [path.join(root, "skill-mission-controller.js"), ...process.argv.slice(2)], {
      cwd: process.cwd(),
      stdio: "inherit"
    });
